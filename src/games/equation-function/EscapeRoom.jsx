    ...
    <div style={{...styles.container, animation: roomShake ? 'shake 0.3s ease-in-out 0s 2' : 'none'}}>
      ...
      <div style={styles.visualRoom}>
        <div style={styles.roomWalls}>
          ...
          <div style={{...styles.escapeDoor, right: `${Math.max(10, 60 - (playerPosition * 0.5))}px`, ...(escapeDoorOpen ? styles.escapeDoorOpen : {})}}><div style={styles.doorFrame}><div style={styles.doorPanel}>{escapeDoorOpen ? <div style={styles.openDoorContent}><span style={styles.openDoorText}>🚪 ESCAPE →</span></div> : <div style={styles.lockedDoorContent}><span style={styles.lockedDoorText}>🔒 LOCKED</span><span style={styles.lockedDoorSubtext}>Move closer!</span></div>}</div></div><div style={styles.doorKnob}></div></div>
          <div style={styles.distanceIndicator}><span>🚶 Distance: {Math.max(0, Math.ceil(distanceToDoor))}%</span><div style={styles.distanceBar}><div style={{...styles.distanceFill, width: `${playerPosition}%`}}></div></div></div>
        </div>
        <div style={{...styles.enemySprite, animation: enemyAttacking ? 'enemyAttack 0.3s ease-in-out' : 'float 2s ease-in-out infinite'}}><div style={styles.enemyAvatar}><span style={styles.enemyEmoji}>{currentEnemy?.emoji || "👾"}</span><div style={styles.enemyNameTag}>{currentEnemy?.name || "Mystery Monster"}</div></div><div style={styles.enemySpeechBubble}><span style={styles.enemyQuote}>"{currentEnemy?.attackMessage || "Answer wrong and I bite you!"}"</span></div></div>
      ...
      <div style={styles.puzzleArea}>
        ...
        <div style={styles.equationCard}>
          ...
          <div style={styles.mcContainer}><div style={styles.mcQuestionText}>{currentPuzzleData.question}</div><div style={styles.optionsContainer}>{currentPuzzleData.options.map((option, idx) => { const letter = String.fromCharCode(65 + idx); return (<button key={idx} onClick={() => setMcSelection(idx)} style={{...styles.optionButton, backgroundColor: mcSelection === idx ? '#4caf50' : '#2a2a4a', border: mcSelection === idx ? '2px solid #ffd93d' : '2px solid #4a4a6a'}}><span style={styles.optionLetter}>{letter}.</span><span style={styles.optionText}>{option}</span></button>); })}</div></div>
          <div style={styles.answerContainer}><button onClick={checkAnswer} style={{...styles.answerSubmitButton, width: '100%'}}>⚔️ DEFEAT ENEMY ⚔️</button></div>
          ...
          <div style={styles.stats}><div style={styles.attempts}>⚔️ Attempts: {attempts}</div><div style={styles.xpStats}>✅ {correctAnswers} | ❌ {wrongAnswers}</div><button onClick={() => setShowHint(!showHint)} style={styles.hintButton}>{showHint ? "Hide Hint" : "Show Hint"}</button></div>
        </div>
        <div style={styles.tipsCard}><h4 style={styles.tipsTitle}>📚 Tips</h4><ul style={styles.tipsList}><li>✓ Slope (rate) vs y-intercept (starting)</li><li>✓ Parallel = same slope</li><li>✓ Negative slope = decreasing</li><li>⭐ +10 XP/correct, -5 XP/wrong</li><li>⚠️ Wrong answer = -1 Life</li><li>❤️ {lives} lives left</li></ul></div>
      </div>
    </div>
  );
};
...
const styles = {
  container: { maxWidth: '100%', width: '100%', margin: 0, padding: '10px', backgroundColor: '#1a1a2e', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.4)', boxSizing: 'border-box', '@media (min-width: 769px)': { maxWidth: '1000px', margin: '20px auto', padding: '20px', borderRadius: '20px' } },
  enemyDefeatOverlay: { position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 1000, pointerEvents: 'none' },
  enemyDefeatBubble: { backgroundColor: '#ffd93d', padding: '8px 12px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 3px 10px rgba(0,0,0,0.3)', flexWrap: 'wrap', justifyContent: 'center', '@media (min-width: 769px)': { padding: '15px 25px', gap: '15px' } },
  ...
  enemyDefeatXp: { fontSize: '10px', color: '#4caf50', fontWeight: 'bold', backgroundColor: 'white', padding: '2px 6px', borderRadius: '12px', '@media (min-width: 769px)': { fontSize: '16px', padding: '4px 10px' } },
  visualRoom: { position: 'relative', backgroundColor: '#2d1b0e', borderRadius: '12px', padding: '10px', marginBottom: '15px', minHeight: '220px', boxShadow: 'inset 0 0 30px rgba(0,0,0,0.5), 0 3px 10px rgba(0,0,0,0.3)', border: '3px solid #5c3a1e', overflow: 'hidden', '@media (min-width: 769px)': { padding: '20px', minHeight: '320px', borderRadius: '16px', borderWidth: '4px' } },
  roomWalls: { position: 'relative', height: '200px', '@media (min-width: 769px)': { height: '280px' } },
  roomWallLeft: { position: 'absolute', left: 0, top: 0, width: '12px', height: '100%', backgroundColor: '#8B6914', background: 'linear-gradient(180deg, #a07828 0%, #6b4c1a 100%)', borderRadius: '3px 0 0 3px', '@media (min-width: 769px)': { width: '20px' } },
  roomWallRight: { position: 'absolute', right: 0, top: 0, width: '12px', height: '100%', backgroundColor: '#8B6914', background: 'linear-gradient(180deg, #a07828 0%, #6b4c1a 100%)', borderRadius: '0 3px 3px 0', '@media (min-width: 769px)': { width: '20px' } },
  roomWallTop: { position: 'absolute', top: 0, left: 0, width: '100%', height: '12px', backgroundColor: '#8B6914', background: 'linear-gradient(90deg, #a07828 0%, #6b4c1a 100%)', borderRadius: '3px 3px 0 0', '@media (min-width: 769px)': { height: '20px' } },
  roomFloor: { position: 'absolute', bottom: 0, left: 0, width: '100%', height: '25px', backgroundColor: '#4a2a0a', background: 'repeating-linear-gradient(45deg, #5c3612, #5c3612 15px, #6b4015 15px, #6b4015 30px)', borderRadius: '0 0 10px 10px', '@media (min-width: 769px)': { height: '40px' } },
  escapeDoor: { position: 'absolute', bottom: '25px', width: '50px', height: '90px', transition: 'all 0.5s ease', cursor: 'pointer', zIndex: 10, '@media (min-width: 769px)': { bottom: '40px', width: '80px', height: '140px' } },
  escapeDoorOpen: { transform: 'translateX(5px)', opacity: 0.9 },
  doorFrame: { width: '100%', height: '100%', backgroundColor: '#3d2b1a', border: '2px solid #8B6914', borderRadius: '5px', boxShadow: 'inset 0 0 5px rgba(0,0,0,0.5)', position: 'relative', '@media (min-width: 769px)': { borderWidth: '3px' } },
  ...
  lockedDoorContent: { color: '#ff6b6b', fontSize: '7px', fontWeight: 'bold', textAlign: 'center', '@media (min-width: 769px)': { fontSize: '10px' } },
  lockedDoorText: { display: 'block', fontSize: '8px', '@media (min-width: 769px)': { fontSize: '14px' } },
  lockedDoorSubtext: { display: 'block', fontSize: '5px', marginTop: '2px', color: '#aaa', '@media (min-width: 769px)': { fontSize: '8px', marginTop: '5px' } },
  openDoorContent: { color: '#4caf50', fontSize: '8px', fontWeight: 'bold', transform: 'rotate(-90deg)', whiteSpace: 'nowrap', '@media (min-width: 769px)': { fontSize: '14px' } },
  openDoorText: { display: 'inline-block' },
  doorKnob: { position: 'absolute', right: '3px', top: '50%', transform: 'translateY(-50%)', width: '6px', height: '6px', backgroundColor: '#ffd700', borderRadius: '50%', '@media (min-width: 769px)': { width: '10px', height: '10px', right: '5px' } },
  distanceIndicator: { position: 'absolute', bottom: '5px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(0,0,0,0.7)', padding: '4px 10px', borderRadius: '12px', fontSize: '8px', color: 'white', textAlign: 'center', zIndex: 15, whiteSpace: 'nowrap', '@media (min-width: 769px)': { bottom: '10px', padding: '8px 15px', fontSize: '12px' } },
  distanceBar: { width: '80px', height: '4px', backgroundColor: '#333', borderRadius: '2px', marginTop: '3px', overflow: 'hidden', '@media (min-width: 769px)': { width: '150px', height: '6px', marginTop: '5px' } },
  distanceFill: { height: '100%', backgroundColor: '#4caf50', transition: 'width 0.3s ease' },
  enemySprite: { position: 'absolute', left: '50%', top: '40%', transform: 'translate(-50%, -50%)', textAlign: 'center', zIndex: 5, '@media (min-width: 769px)': { top: '50%' } },
  enemyAvatar: { backgroundColor: '#2a1a0a', borderRadius: '50%', width: '50px', height: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #ff4444', boxShadow: '0 0 8px rgba(255,0,0,0.5)', '@media (min-width: 769px)': { width: '80px', height: '80px', borderWidth: '3px' } },
  enemyEmoji: { fontSize: '28px', '@media (min-width: 769px)': { fontSize: '42px' } },
  enemyNameTag: { fontSize: '7px', backgroundColor: '#ff4444', padding: '1px 4px', borderRadius: '8px', marginTop: '2px', color: 'white', fontWeight: 'bold', '@media (min-width: 769px)': { fontSize: '10px', padding: '2px 8px', marginTop: '4px' } },
  enemySpeechBubble: { position: 'absolute', top: '-35px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'white', padding: '4px 8px', borderRadius: '12px', whiteSpace: 'nowrap', boxShadow: '0 2px 8px rgba(0,0,0,0.2)', fontSize: '8px', fontWeight: 'bold', color: '#333', '@media (min-width: 769px)': { top: '-50px', padding: '8px 12px', fontSize: '11px' } },
  enemyQuote: { fontSize: '8px', '@media (min-width: 769px)': { fontSize: '11px' } },
  playerSprite: { position: 'absolute', bottom: '35px', textAlign: 'center', zIndex: 5, transition: 'left 0.3s ease', '@media (min-width: 769px)': { bottom: '60px' } },
  playerAvatar: { backgroundColor: '#1a3a5c', borderRadius: '50%', width: '40px', height: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #4caf50', boxShadow: '0 0 8px rgba(76,175,80,0.5)', '@media (min-width: 769px)': { width: '60px', height: '60px', borderWidth: '3px' } },
  playerEmoji: { fontSize: '22px', '@media (min-width: 769px)': { fontSize: '32px' } },
  playerNameTag: { fontSize: '6px', backgroundColor: '#4caf50', padding: '1px 4px', borderRadius: '8px', marginTop: '1px', color: 'white', fontWeight: 'bold', '@media (min-width: 769px)': { fontSize: '8px', padding: '1px 6px', marginTop: '2px' } },
  bloodEffect: { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '30px', opacity: 0.8, animation: 'fadeOut 0.5s ease-out', pointerEvents: 'none', zIndex: 20, '@media (min-width: 769px)': { fontSize: '60px' } },
  puzzleArea: { marginTop: '15px', '@media (min-width: 769px)': { marginTop: '20px' } },
  header: { marginBottom: '15px' },
  scoreTimeContainer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', fontSize: '11px', fontWeight: 'bold', flexWrap: 'wrap', gap: '6px', '@media (min-width: 769px)': { fontSize: '18px', marginBottom: '10px', gap: '10px' } },
  ...
  livesContainer: { color: '#ff4444', background: 'rgba(0,0,0,0.3)', padding: '3px 8px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '3px', '@media (min-width: 769px)': { padding: '5px 12px', gap: '5px' } },
  xpDisplay: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', marginBottom: '8px', padding: '5px', backgroundColor: 'rgba(255,215,0,0.2)', borderRadius: '12px', fontSize: '9px', fontWeight: 'bold', color: '#ffd93d', flexWrap: 'wrap', '@media (min-width: 769px)': { gap: '10px', marginBottom: '10px', padding: '8px', fontSize: '14px' } },
  xpBreakdown: { fontSize: '7px', color: '#aaa', '@media (min-width: 769px)': { fontSize: '11px' } },
  progressBar: { width: '100%', height: '6px', backgroundColor: '#1e1e2e', borderRadius: '3px', overflow: 'hidden', marginBottom: '5px', '@media (min-width: 769px)': { height: '8px', marginBottom: '8px' } },
  ...
  equationCard: { backgroundColor: '#1e1e2e', borderRadius: '12px', padding: '15px', textAlign: 'center', '@media (min-width: 769px)': { padding: '30px', borderRadius: '16px' } },
  difficultyBadge: { display: 'inline-block', padding: '2px 8px', borderRadius: '12px', fontSize: '8px', fontWeight: 'bold', marginBottom: '15px', color: '#fff', textTransform: 'uppercase', '@media (min-width: 769px)': { padding: '4px 12px', fontSize: '11px', marginBottom: '25px' } },
  mcContainer: { marginBottom: '15px' },
  mcQuestionText: { fontSize: '14px', color: '#fff', marginBottom: '15px', lineHeight: '1.4', whiteSpace: 'pre-line', textAlign: 'center', '@media (min-width: 769px)': { fontSize: '20px', marginBottom: '25px' } },
  optionsContainer: { display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '15px', '@media (min-width: 769px)': { gap: '12px', marginBottom: '20px' } },
  optionButton: { display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', backgroundColor: '#2a2a4a', border: '2px solid #4a4a6a', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left', fontSize: '11px', color: '#fff', '@media (min-width: 769px)': { gap: '15px', padding: '15px 20px', fontSize: '16px' } },
  optionLetter: { fontWeight: 'bold', fontSize: '12px', minWidth: '20px', color: '#ffd93d', '@media (min-width: 769px)': { fontSize: '18px', minWidth: '30px' } },
  optionText: { flex: 1, fontSize: '10px', '@media (min-width: 769px)': { fontSize: '14px' } },
  answerContainer: { marginBottom: '15px', padding: '12px', backgroundColor: '#0f0f1f', borderRadius: '10px', border: '2px solid #ff9800', '@media (min-width: 769px)': { marginBottom: '25px', padding: '25px', borderRadius: '16px' } },
  answerSubmitButton: { padding: '10px 16px', fontSize: '12px', backgroundColor: '#ff4444', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.3s', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', minHeight: '44px', '@media (min-width: 769px)': { padding: '18px 24px', fontSize: '18px', letterSpacing: '2px' } },
  feedback: { padding: '8px', marginBottom: '10px', backgroundColor: '#2a2a4a', borderRadius: '8px', fontSize: '10px', lineHeight: '1.4', color: '#fff', '@media (min-width: 769px)': { padding: '12px', fontSize: '14px' } },
  feedbackDeath: { padding: '8px', marginBottom: '10px', backgroundColor: '#8b0000', borderRadius: '8px', fontSize: '10px', lineHeight: '1.4', color: '#fff', fontWeight: 'bold', '@media (min-width: 769px)': { padding: '12px', fontSize: '14px' } },
  hint: { padding: '8px', marginBottom: '10px', backgroundColor: '#ff9800', borderRadius: '8px', fontSize: '10px', color: '#fff', '@media (min-width: 769px)': { padding: '12px', fontSize: '14px' } },
  stats: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #3a3a5a', flexWrap: 'wrap', gap: '8px', '@media (min-width: 769px)': { marginTop: '15px', paddingTop: '15px', gap: '10px' } },
  attempts: { fontSize: '9px', color: '#aaa', '@media (min-width: 769px)': { fontSize: '13px' } },
  xpStats: { fontSize: '9px', color: '#ffd93d', fontWeight: 'bold', '@media (min-width: 769px)': { fontSize: '13px' } },
  hintButton: { padding: '4px 10px', fontSize: '9px', backgroundColor: '#4a6fa5', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', minHeight: '32px', '@media (min-width: 769px)': { padding: '6px 14px', fontSize: '12px' } },
  tipsCard: { backgroundColor: '#1e1e2e', borderRadius: '10px', padding: '10px', '@media (min-width: 769px)': { padding: '18px', borderRadius: '12px' } },
  tipsTitle: { fontSize: '11px', marginBottom: '8px', color: '#ffd93d', '@media (min-width: 769px)': { fontSize: '16px', marginBottom: '12px' } },
  tipsList: { listStyle: 'none', padding: 0, margin: 0, fontSize: '8px', lineHeight: '1.5', color: '#ccc', '@media (min-width: 769px)': { fontSize: '13px', lineHeight: '1.8' } },
  completionContainer: { display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px', padding: '12px', '@media (min-width: 769px)': { minHeight: '400px', padding: '20px' } },
  completionCard: { backgroundColor: '#2a2a4a', borderRadius: '16px', padding: '20px', textAlign: 'center', maxWidth: '300px', width: '90%', '@media (min-width: 769px)': { padding: '35px', maxWidth: '450px', borderRadius: '20px' } },
  trophyIcon: { fontSize: '36px', marginBottom: '10px', '@media (min-width: 769px)': { fontSize: '56px', marginBottom: '15px' } },
  sadIcon: { fontSize: '36px', marginBottom: '10px', '@media (min-width: 769px)': { fontSize: '56px' } },
  completionTitle: { fontSize: '20px', marginBottom: '10px', color: '#ffd93d', '@media (min-width: 769px)': { fontSize: '28px', marginBottom: '15px' } },
  completionText: { fontSize: '11px', marginBottom: '15px', color: '#ccc', '@media (min-width: 769px)': { fontSize: '16px', marginBottom: '20px' } },
  finalScore: { backgroundColor: '#1e1e2e', padding: '10px', borderRadius: '8px', marginBottom: '15px', fontSize: '10px', lineHeight: '1.5', '@media (min-width: 769px)': { padding: '15px', fontSize: '14px', lineHeight: '1.8' } },
  continueButton: { padding: '8px 18px', fontSize: '12px', backgroundColor: '#4caf50', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', minHeight: '40px', '@media (min-width: 769px)': { padding: '10px 25px', fontSize: '14px' } }
};
