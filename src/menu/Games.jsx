...
function Games() {
  ...
  // Calculate total scores from game progress
  ...
  // Show notification
  const showNotification = (message, color, xpAmount = null) => {
    const id = Date.now();
    setNotification({ id, message, color, xpAmount });
    setTimeout(() => setNotification(prev => prev?.id === id ? null : prev), 3000);
  };

  // Update XP function with proper state updates
  const updateXP = (points, reason = '') => {
    ...
    if (user?.email) {
      localStorage.setItem(`userXP_${user.email}`, newXP.toString());
    }
    localStorage.setItem('userXP', newXP.toString());
    
    // Update userData if function exists
    ...
    if (points > 0) {
      showNotification(`+${points} XP! ${reason}`, '#10b981', points);
    } else if (points < 0) {
      showNotification(`${points} XP (${reason})`, '#ef4444', points);
    }
  };

  // Save game score and calculate XP based on performance
  const saveGameScore = (gameId, score, completed = false, stats = {}) => {
    setGameProgress(prevProgress => {
      ...
      const shouldIncrementAttempts = !currentGameProgress.lastPlayed || 
                                      Date.now() - new Date(currentGameProgress.lastPlayed).getTime() > 60000;
      
      // Calculate XP from correct/wrong answers if stats provided
      if (stats.correctAnswers !== undefined || stats.wrongAnswers !== undefined) {
        ...
        if (xpGain !== 0) {
          updateXP(xpGain, `${gameId}: ${correctAnswers} correct, ${wrongAnswers} wrong`);
        }
      }
      
      // Bonus XP for first completion
      if (completed && !currentGameProgress.completed) {
        updateXP(50, `Completed ${gameId}! 🎉`);
        showNotification(`🎉 Bonus 50 XP for completing ${gameId}!`, '#f59e0b', 50);
      }
      ...
      if (score > currentGameProgress.highScore && currentGameProgress.highScore > 0) {
        ...
        showNotification(`🏆 New High Score! +${bonusXP} XP!`, '#f59e0b', bonusXP);
      }
  ...
  useEffect(() => {
    // Load XP from various sources
    let loadedXP = 0;
    
    if (userData?.xp !== undefined && userData?.xp !== null) {
      loadedXP = userData.xp;
    } else if (user?.email) {
      const savedXP = localStorage.getItem(`userXP_${user.email}`);
      if (savedXP) loadedXP = parseInt(savedXP, 10) || 0;
    }
    
    if (loadedXP === 0) {
      const generalXP = localStorage.getItem('userXP');
      if (generalXP) loadedXP = parseInt(generalXP, 10) || 0;
    }
    
    setXpPoints(loadedXP);
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      // Accept messages from same origin or game iframes
      ...
      if (event.data.type === 'GAME_RESULT') {
        ...
        setCurrentScore(score);
        if (completed) {
          showNotification(`🏆 Completed! Final Score: ${score}`, '#10b981');
      ...
      // Handle individual answer results (real-time XP)
      if (event.data.type === 'ANSWER_RESULT') {
      ...
      // Handle direct XP update from game
      if (event.data.type === 'UPDATE_XP') {
        const { points, reason } = event.data;
        updateXP(points, reason);
      }
    };
  ...
  const resetAllProgress = () => {
    ...
    if (resetXP) {
      ...
      if (updateUserData && userData) {
        updateUserData({ ...userData, gameProgress: resetProgress, xp: 0, totalScores: 0 });
      }
      showNotification('🗑️ All progress and XP reset!', '#10b981');
    } else {
      if (updateUserData && userData) {
        updateUserData({ ...userData, gameProgress: resetProgress });
      }
        ...
        <div style={styles.statsGrid}>
          ...
          <div style={styles.statCard}>
            ...
            <div style={styles.statCardInfo}>
              ...
              <div style={styles.statCardLabel}>Total Score</div>
            </div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statCardIcon}>🎯</div>
            <div style={styles.statCardInfo}>
              ...
              <div style={styles.statCardLabel}>Attempts</div>
            </div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statCardIcon}>📊</div>
            <div style={styles.statCardInfo}>
              ...
              <div style={styles.statCardLabel}>Progress</div>
            </div>
          </div>
        </div>
      ...
      <div style={styles.cardContainer}>
        <div style={styles.cardContent}>
          <div style={styles.cardLeft}>
            <div style={{...styles.cardIcon, backgroundColor: game.bgColor, color: game.color}}>
              {game.icon}
            </div>
            <div style={styles.cardText}>
              ...
              <div style={styles.featuresList}>
                {game.features.map((f, i) => <span key={i} style={styles.featureTag}>{f}</span>)}
              </div>
              <div style={styles.cardMeta}>
                <span style={styles.timeEstimate}>⏱️ {game.timeEstimate}</span>
                {progress?.completed && <span style={styles.completedBadge}>✓ Completed</span>}
                {highScore > 0 && <span style={styles.bestScore}>🏆 Best: {highScore}</span>}
              </div>
            </div>
          </div>
          <button style={styles.startButton} onClick={() => onStart(game.id)}>
            <FaPlay style={styles.playIcon} /> Play Game
          </button>
        </div>
      </div>
    );
  ...
  const games = {
    equation: { 
      ...
      description: "Solve linear equations to escape each room!", 
      icon: <GiPuzzle size={36} />, 
      color: "#3b82f6", 
      bgColor: "#dbeafe", 
      difficulty: "Easy", 
      timeEstimate: "15 min", 
      features: ["Levels", "Timed", "Hints"], 
      path: "/game/equation" 
    },
    battle: { 
      id: 'battle', 
      title: "Math Battle", 
      description: "Epic turn-based combat! Solve equations to attack!", 
      icon: <GiSwordsEmblem size={36} />, 
      color: "#8b5cf6", 
      bgColor: "#ede9fe", 
      difficulty: "Hard", 
      timeEstimate: "25 min", 
      features: ["Combat", "Power-ups", "Bosses"], 
      path: "/game/battle" 
    },
    spaceShooter: { 
      id: 'spaceShooter', 
      title: "Space Shooter", 
      description: "Defend your ship by solving math problems!", 
      icon: <GiConsoleController size={36} />, 
      color: "#f59e0b", 
      bgColor: "#fef3c7", 
      difficulty: "Medium", 
      timeEstimate: "20 min", 
      features: ["Action", "Upgrades", "Enemies"], 
            ...
            <div style={styles.gameModalTitleSection}>
              ...
              <div>
                ...
                <p style={styles.gameModalSubtitle}>
                  {currentScore !== null ? 
                    <span style={{color:'#ffd93d'}}>🎯 Current Score: {currentScore}</span> : 
                    '+10 XP per correct, -5 XP per wrong answer'}
                </p>
              </div>
            </div>
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Notification */}
      {notification && (
        <div style={{...styles.notification, backgroundColor: notification.color}}>
      ...
      {showResetConfirm && (
        <div style={styles.modalOverlay} onClick={() => setShowResetConfirm(false)}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={styles.modalText}>This cannot be undone. All progress and scores will be deleted.</p>
...
const styles = {
  container: { 
    ...
    '@media (min-width: 769px)': { padding: '24px', maxWidth: '1400px', margin: '0 auto' } 
  },
  dashboardContainer: { 
    background: 'white', 
    borderRadius: '12px', 
    ...
    '@media (min-width: 769px)': { padding: '24px', marginBottom: '30px', borderRadius: '16px' } 
  },
  dashboardHeader: { 
    display: 'flex', 
    alignItems: 'center', 
    ...
    flexWrap: 'wrap', 
    '@media (min-width: 640px)': { gap: '12px' } 
  },
  ...
  xpDisplay: { 
    ...
    borderRadius: '16px', 
    fontWeight: 'bold', 
    '@media (min-width: 769px)': { padding: '8px 16px', gap: '8px' } 
  },
  ...
  resetButton: { 
    ...
    fontWeight: '500', 
    minHeight: '36px', 
    '@media (min-width: 769px)': { padding: '8px 16px', fontSize: '14px', gap: '8px' } 
  },
  ...
  statsGrid: { 
    ...
    gap: '8px', 
    marginBottom: '16px', 
    '@media (min-width: 640px)': { gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }, 
    '@media (min-width: 769px)': { gap: '15px', marginBottom: '30px' } 
  },
  statCard: { 
    ...
    alignItems: 'center', 
    gap: '10px', 
    '@media (min-width: 769px)': { padding: '20px', gap: '15px' } 
  },
  ...
  dropdownHeader: { 
    ...
    color: '#1f2937', 
    fontFamily: 'inherit', 
    '@media (min-width: 769px)': { padding: '16px 20px', fontSize: '16px' } 
  },
  ...
  gamesGrid: { 
    ...
    gap: '12px', 
    '@media (min-width: 860px)': { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '20px' } 
  },
  cardContainer: { 
    background: 'white', 
    borderRadius: '14px', 
    ...
    cursor: 'pointer',
    '@media (min-width: 769px)': { padding: '24px', borderRadius: '16px' } 
  },
  ...
  startButton: { 
    ...
    '@media (min-width: 769px)': { padding: '14px', fontSize: '15px', borderRadius: '12px', gap: '8px' } 
  },
  ...
  gameModalContainer: { 
    ...
    '@media (min-width: 769px)': { width: '95%', maxWidth: '1400px', height: '90%', borderRadius: '20px' } 
  },
  gameModalHeader: { 
    padding: '12px 16px', 
  ...
  closeButton: { 
    ...
    width: '32px', 
    height: '32px', 
    '@media (min-width: 769px)': { fontSize: '20px', width: '40px', height: '40px' } 
  },
  ...
  modalContent: { 
    ...
    maxWidth: '320px', 
    width: '90%', 
    '@media (min-width: 769px)': { borderRadius: '16px', padding: '24px', maxWidth: '400px' } 
  },
  ...
  notification: { 
    ...
    textAlign: 'center', 
    fontSize: '12px', 
    '@media (min-width: 481px)': { bottom: '20px', right: '20px', left: 'auto', padding: '12px 20px', fontSize: '14px' } 
  }
};
...
styleSheet.textContent = `
  ...
  @media (max-width: 480px) { 
    button, .optionLabel { min-height: 40px; } 
  } 
  @media (max-width: 360px) { 
    .dashboardTitle { font-size: 14px !important; } 
    .statCardValue { font-size: 16px !important; } 
    .statCardLabel { font-size: 8px !important; } 
    .cardTitle { font-size: 14px !important; } 
  }
`;
if (!document.querySelector('#games-responsive-styles')) { 
  styleSheet.id = 'games-responsive-styles'; 
